...
class PromptServer():
    def __init__(self, loop):
        ...
        async def download_paint3d(request):
            filename = request.match_info.get("filename", "")
            paint3d_dir = os.path.join("/root/ComfyUI/input/3d/Paint3D")
            
            if not filename:
                return web.Response(status=400, text="Filename is required")
        
            file_path = os.path.join(paint3d_dir, filename)
            
            # Security check: Ensure the file path is within the Paint3D directory
            if not os.path.commonpath([os.path.abspath(file_path), paint3d_dir]) == paint3d_dir:
